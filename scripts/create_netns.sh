...
ip netns exec aftr  ip link add name br0 type bridge
ip netns exec aftr  ip link set dev br0 up
ip netns exec aftr  ip link add name eth0 type dummy
ip netns exec aftr  ip link set eth0 master br0
ip netns exec aftr  ip link set dev eth0 up
ip netns exec aftr  ip addr add 1.1.1.1/24 dev eth0

ip netns exec aftr  ip link set dev veth8 up
ip netns exec aftr  ip link set veth8 master br0
ip netns exec aftr  ip addr add 2401:db00::3/64 dev br0
...
ip netns exec host1 dnsmasq \
    --enable-ra \
    --dhcp-range=::,constructor:veth0,ra-stateless,ra-names,ra-advrouter \
...
ip -6 route add 2401:db00::/64 via 2001:db8::1 dev veth1
